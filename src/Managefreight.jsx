...
export default function Managefreight() {
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      if (
        ...
        toast.error(json.message || "Failed to export freight report.");
        return;
      }

      let filename = `Freight_Admin_Report_${Date.now()}.xlsx`;
  ...
  return (
    <>
      ...
      {/* Export Freight Admin Report Modal */}
      <Modal
      ...
      >
        ...
        >
          ...
          {(searchQuery || (status1 && status1 !== "Status")) && (
            <div className="alert alert-light border p-2 mb-3 small text-muted">
              {status1 && status1 !== "Status" && (
                <div>
                  <strong>Status:</strong>{" "}
                  <span className="badge bg-primary">{status1}</span>
                </div>
              )}
              {searchQuery && (
                <div
                  className={
                    status1 && status1 !== "Status" ? "mt-1" : ""
                  }
                >
